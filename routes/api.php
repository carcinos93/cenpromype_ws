...
# TODO mover a middler auditoria
Route::prefix('portal')->group(function () {
    Route::get('usuarios',  'PortalController@usuarios');
    Route::get('respuestas/{idusuario}',  'PortalController@respuestas');
    Route::post('usuario-descargas',  'PortalController@usuarioDescargaInsert');
    Route::get('usuario-descargas',  'PortalController@usuarioDescargaAll');
});
...
Route::prefix('catalogos')->group(function () {
    ...
    Route::get('/catalogo/{tipo}/{nivel}', function ($tipo, $nivel) {
        ...
        if ($group == '1') {
            $catalogos = $catalogos->with('items');
        }
    ...
    Route::get('/formulario/pregunta/{id}', function ($id) {
        ...
        ->where('VALOR_DEPENDE', 'LIKE', "%{$valor}%" )
        ->first();
        if ( $formulario ) {
            return $formulario;
           /* if ($formulario['VALOR_DEPENDE'] != null) {
                $valores = explode(",", $formulario['VALOR_DEPENDE']);
                return $valores;
                return in_array( $valor, $valores ) ? $formulario : [ 'preguntas' => [] ];
            }*/
        } else {
            return [ 'preguntas' => [] ];
        }
    ...
    Route::middleware(['auditoria'])->group(function () {
        ...
        //FORMULARIO
        Route::get('/formulario', 'CatalogosController@FormularioAll');
        Route::post('/formulario', 'CatalogosController@FormularioInsert');
        Route::put('/formulario/{id}', 'CatalogosController@FormularioUpdate');
        Route::delete('/formulario/{id}', 'CatalogosController@FormularioDelete');
...
Route::prefix('listas')->group(function () {
    ...
    Route::get('/formularios', 'CatalogosController@FormulariosLista');
     Route::middleware(['auditoria'])->group(function () {
